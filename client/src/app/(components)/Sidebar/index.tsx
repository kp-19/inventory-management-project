...
import { Menu,LucideIcon, Layout, Archive, Clipboard, User, SlidersHorizontal, DollarSign } from 'lucide-react';
    ...
    return(
        <Link href={href}>
            <div className={`cursor-pointer flex items-center ${isCollapsed ? "justify-center py-4" : "justify-start px-8 py-4"} hover:text-blue-500 hover:bg-blue-100 gap-3 transition-colors ${isActive ? "bg-blue-200 text-white" : ""}`}>
                ...
                <span className={`${isCollapsed ? "hidden":"blocl"} font-medium text-gray-700`}>
                    {label}
                </span>
            </div>
        </Link>
...
const Sidebar = () => {
    ...
    const sidebarClassNames=`fixed flex flex-col ${isSidebarCollapsed ? "w-0 md:w-14" : "w-72 md:w-72" } bg-white transition-all duration-200 overflow-hidden h-full shadow-md z-40`;
    return (
    <div className={sidebarClassNames}>
        <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${ isSidebarCollapsed ? "px-5" : "px-8"}`}>
            ...
            <h1 className={`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-2xl`}>Stocky</h1>
        ...
        <div className='flex-grow mt-8'>
            ...
            <SidebarLink href="/expenses" icon={DollarSign}  label="Expenses" isCollapsed={isSidebarCollapsed} />
        ...
        <div className={`${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
            <p className='text-center  text-xs text-gray-500'>&copy; 2024 Stocky</p>
        </div>
    </div>
